...
import ShirtPageProps from "./interfaces/ShirtPageProps";
import Shirt from "./interfaces/Shirt";

async function ShirtPage({ params, searchParams }: ShirtPageProps) {
  const shirt: Shirt = (await axios.get(`http://localhost:5555/products-api/clothes/shirts/${params.id}`)).data;
  ...
  return (
    <Container className="mt-4">
      ...
      <FlexCol className="gap-2 mt-2">
        ...
        {view === "basic" ? (
          <IBasicView name={shirt.name} brand={shirt.brand} price={shirt.price} />
        ) : (
          <IAdvancedView name={shirt.name} brand={shirt.brand} price={shirt.price} />
        )}
      </FlexCol>
